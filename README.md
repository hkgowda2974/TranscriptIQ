...
```text
 1. INGESTION                2. SEARCH & INDEX          3. SAFETY GATE & ANSWER            4. VERIFIED UI
┌────────────────────┐      ┌────────────────────┐     ┌────────────────────────┐      ┌────────────────────────┐
│  Raw Transcripts   │      │ Turn-Aware Chunks  │     │ User Question          │      │  TranscriptIQ Web App  │
│  & Interview Guide │ ───► │ + Exact Timestamps │ ──► │           ▼            │ ───► │  • Streaming Answer    │
└────────────────────┘      │ + Vector Store     │     │ Evidence Check Gate?   │      │  • Verbatim Quotes     │
                            └────────────────────┘     │  ├─ No  ► Safe Refusal │      │  • Speaker & Timestamps│
                                                       │  └─ Yes ► Grounded Ans │      │  • Synthesis Matrix    │
                                                       └────────────────────────┘      └────────────────────────┘
```
